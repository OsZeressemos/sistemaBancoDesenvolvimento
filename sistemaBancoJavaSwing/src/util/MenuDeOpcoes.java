...
public class MenuDeOpcoes {
	...
	public static void menuCliente(int cpfLogin, Map<Integer, Conta> mapConta, Map<Integer, Cliente> mapCliente,
		...
		ImageIcon icon = new ImageIcon("E:\\CURSOS\\Digital Innovation One\\jogo da cobrinha\\img\\favicon\\snake2");
		...
		while (controle == 1) {
			...
			if (conta1.getTipo() == 1) {
				...
				String entrada = JOptionPane.showInputDialog(icon + "------- TIPO DE OPERAÇÃO -------" + "\n\n 1 - Depósito"
				...
				if (saida == 1) {
					...
					System.out.printf("Total de tarifas cobradas: R$ %.2f%n", (conta.consultaTotalTaxas() + (valorSeguro * 0.2)));
	...
	public static void menuDiretor(int cpfLogin, Map<Integer, Conta> mapConta, Map<Integer, Cliente> mapCliente,
		...
		while (controle == 1) {
			...
			if (conta1.getTipo() == 2) {
				...
				String entrada = JOptionPane.showInputDialog("------- TIPO DE OPERAÇÃO -------" + "\n\n 1 - Depósito"
						+ "\n 2 - Saque" + "\n 3 - Simulação rendimento poupança" + "\n 4 - Transferência" + "\n 5 - Consulta Saldo"
						+ "\n 6 - Contratar Seguro de Vida" + "\n 7 - Relatório Gerente" + "8 - Relatório Diretor" + "\n 9 - Sair"
						+ "\n                     ZeressemosBank\n");
				int saida = Integer.parseInt(entrada);
				if (saida == 1) {
					String entradaDeposito = JOptionPane.showInputDialog("Digite o valor que deseja depositar:");
